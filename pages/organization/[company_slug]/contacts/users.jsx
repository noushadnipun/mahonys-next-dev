...
const Users = ({userDetails}) =>{
    ...
    const handleSave = async (e) => {
        ...
        let obj = {
            ...
            designation_id: target.designation_id.value,
            organization_id: userDetails?.organization_id,
            company_id: userDetails?.company_id
        }
        let route = editData ? '/user/update' : '/user/create'
    ...
    return (
        <>
            ...
            <section className="row">
                <h3 className="page-headline">{total || 'List of '} Users</h3>
...
const Lists = ({datas, onEditItem}) => {
    ...
    return (
        <>
            <Table props={{
                tableHeads: ["Name", "Designation", "Email", "Phone", "Action"],
                tableBody: tableBody() ?? []
            }} />
        </>
    )
...
const FormData = ({data, modalShow, modalHide, save, modalHeader}) => {
    ...
    const loadDesignation = async () => {
        let types = await getDesignations();
...
export const getDesignations = async () => {
    let res = await axios.get("company/designations?sort=0");
    // console.log(res)
    res = res.data.getData
    let data = [];
    res.map((item) => {
        let obj = {
